...
async function checkChannelMembership(ctx) {
    const channelId = "@stxpos";
    try {
        const chatMember = await ctx.telegram.getChatMember(channelId, ctx.from.id);
        log("Status member:", chatMember.status); // <--- cek status user
        return ["member", "administrator", "creator"].includes(chatMember.status);
    } catch (error) {
        log("Gagal memeriksa keanggotaan channel:", error.message);
        return false;
    }
}
...
┃ ☍ ʟᴇᴠᴇʟs : Low | Medium | High [ DEFAULT : HIGH ]
║ ☍ ᴄʜᴀɴɴᴇʟ : @Stxpos
┃ ☍ ᴅᴇᴠᴇʟᴏᴘᴇʀ : @Sniith
╚━═━═━═━═━═━═━═━═━═━❍
...
users.add(ctx.from.id);
    ...
    if (!isMember) {
        return ctx.replyWithMarkdown(
            ...
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://t.me/stxpos)"
        );
    }

    if (!ctx.message.reply_to_message || !ctx.message.reply_to_message.document) {
        return ctx.replyWithMarkdown("❌ *Error:* Balas file .js dengan `/violence [low/medium/high]`!");
    }
